...
public class TCPPacket extends IPPacket
{
	...
	/** Offset into the TCP packet of the acknowledgment number header value. */
	...
	/**
	 * Clears all selective acknowledgment options. This is a temporary kluge and will be removed from the final API. Do
	 * not use it. The final API will have proper methods for adjusting selective acknowledgment options.
	 */
		...
		if (headerLength > offset)
		{
			...
			loop: do
			{
				...
				default:
					...
					/*
					 * int length = _data_[offset + 1]; while(length-- > 0) _data_[offset++] = KIND_NOP;
					 */
					break;
				}
	...
	/**
	 * Copies the contents of a TCPPacket. If the current data array is of insufficient length to store the contents, a
	 * new array is allocated.
	 * 
	 * @param packet
	 *            The TCPPacket to copy.
	 */
	...
	{
		super.setData(data);
		__offset = getIPHdrOffset() + getIPHeaderByteLength();
	}
	 ...
	 *            Indicates whether the raw data includes the Ethernet header otherwise the first byte is the beginning
	 *            of the IP header.
	 */
	@Override
	public void setData(byte[] data, boolean isEthHdrIcluded)
	{
		super.setData(data, isEthHdrIcluded);
		__offset = getIPHdrOffset() + getIPHeaderByteLength();
	}
	...
	{
		return (((_data_[__offset + OFFSET_DESTINATION_PORT] & 0xff) << 8) | (_data_[__offset + OFFSET_DESTINATION_PORT
				+ 1] & 0xff));
	}
	...
	public final long getSequenceNumber()
	{
		return (((_data_[__offset + OFFSET_SEQUENCE] & 0xffL) << 24)
				...
				| ((_data_[__offset + OFFSET_SEQUENCE + 2] & 0xffL) << 8) | (_data_[__offset + OFFSET_SEQUENCE + 3] & 0xffL));
	}
	 ...
	 * 
	 * @param seq
	 *            The new acknowledgement number.
	 */
	...
	{
		return (((_data_[__offset + OFFSET_ACK] & 0xffL) << 24) | ((_data_[__offset + OFFSET_ACK + 1] & 0xffL) << 16)
				| ((_data_[__offset + OFFSET_ACK + 2] & 0xffL) << 8) | (_data_[__offset + OFFSET_ACK + 3] & 0xffL));
	}
	...
	{
		return getEthHeaderByteLength() + getIPHeaderByteLength() + getTCPHeaderByteLength();
	}
	 ...
	 */
	public final int getIPAndTCPHeaderByteLength()
	{
	...
	public final void setTCPDataByteLength(int length)
	{
		...
		setIPPacketLength(getIPAndTCPHeaderByteLength() + length);
	}
	...
	{
		return getIPPacketLength() - getIPAndTCPHeaderByteLength();
	}

	private final int __getVirtualHeaderTotal()
	{
		int s1 = ((_data_[getIPHdrOffset() + OFFSET_SOURCE_ADDRESS] & 0xff) << 8)
				| (_data_[getIPHdrOffset() + OFFSET_SOURCE_ADDRESS + 1] & 0xff);
		int s2 = ((_data_[getIPHdrOffset() + OFFSET_SOURCE_ADDRESS + 2] & 0xff) << 8)
				| (_data_[getIPHdrOffset() + OFFSET_SOURCE_ADDRESS + 3] & 0xff);
		int d1 = ((_data_[getIPHdrOffset() + OFFSET_DESTINATION_ADDRESS] & 0xff) << 8)
				| (_data_[getIPHdrOffset() + OFFSET_DESTINATION_ADDRESS + 1] & 0xff);
		int d2 = ((_data_[getIPHdrOffset() + OFFSET_DESTINATION_ADDRESS + 2] & 0xff) << 8)
				| (_data_[getIPHdrOffset() + OFFSET_DESTINATION_ADDRESS + 3] & 0xff);
	...
	{
		return _computeChecksum_(__offset, __offset + OFFSET_TCP_CHECKSUM, getIPHdrOffset() + getIPPacketLength(),
